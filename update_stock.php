...
if (!$result || $result->num_rows == 0) {
    echo "<script>alert('Item not found'); window.location.href='manage_raw_material.php';</script>";
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($add_qty > 0) {
        ...
        $update_sql = "UPDATE stock SET quantity=$new_qty WHERE id=$id";
        if ($conn->query($update_sql) === TRUE) {
            $redirect = ($item['category']=='Raw Material') ? 'manage_raw_material.php' : 'manage_product.php';
            echo "<script>alert('Stock updated successfully!'); window.location.href='$redirect';</script>";
            exit;
        } else {
...
<title>Update Stock | Bitsy Bakesy</title>
...
.card { background:white; padding:30px; border-radius:10px; box-shadow:0 3px 8px rgba(0,0,0,0.2); max-width:500px; margin:auto; }

/* ===== Form ===== */
label { display:block; font-weight:600; margin-top:10px; }
input[type=number] { width:100%; padding:8px; margin-top:5px; border:1px solid #ccc; border-radius:6px;}
button { margin-top:20px; width:100%; padding:10px; border:none; background:#27ae60; color:white; border-radius:6px; cursor:pointer; transition:0.3s;}
...
<!-- ===== Main ===== -->
<div class="main">
<h2>Update Stock - <?= htmlspecialchars($item['item_name']) ?></h2>
...
<div class="card">
    <form method="POST">
        ...
        <input type="number" id="add_qty" name="add_qty" min="1" required>
